        ...
        if lon_rho is None or lat_rho is None:
            ...
            if state["gj"] is not None and state["feature_indices"] is not None:
                ...
                def on_export(event):
                    ...
                    # Update properties with i (xi) and j (eta)
                    ...
                    if TK_AVAILABLE:
                        try:
                            root = tk.Tk()
                            root.withdraw()
                            file_path = filedialog.ask_saveasfilename(
                        ...
                        logging.warning(
                            "tkinter not available; exporting to default path."
                        )
                        file_path = os.path.join(initial_dir, default_name)
                    ...
                    try:
                        ...
                        logging.info(
                            f"Exported modified GeoJSON with i/j to: {file_path}"
                        )
                    except Exception as e:
